...
// From binded texture
layout (location = 55) uniform sampler2D s;
layout (location = 56) uniform sampler2D shadowMap;
...
float LinearizeDepth(float depth)
{
    ...
    return (2.0 * 2.0 * 50.0) / (50.0 + 2.0 - z * (50.0 - 2.0));
}
...
vec3 accumPointLight(vec3 viewDir, vec3 curlightPos, vec3 curlightColor, float curlightMaxDistance)
{
	...
	else
	{
        ...
        vec3 specular = max(specularStrength * spec * curlightColor, vec3(0.0)) * vec3(0.5, 0.5, 0.0);
